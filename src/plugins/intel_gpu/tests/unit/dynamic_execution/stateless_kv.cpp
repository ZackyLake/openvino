...
TEST_P(stateless_kv_runtime, output_memory_reuse) {
    ...
    const auto logical_present_len =
        params.is_present_len ? params.seq_len : params.seq_len + params.new_token_len;
    ...
    const auto past_layout =
        layout{ov::PartialShape{batch, heads, past_capacity, head_size}, data_types::f32, format::bfyx};
    const auto new_token_layout =
        layout{ov::PartialShape{batch, heads, params.new_token_len, head_size}, data_types::f32, format::bfyx};
    const auto seq_len_layout = layout{ov::PartialShape{1}, data_types::i64, format::bfyx};
    const auto pos_idx_layout =
        layout{ov::PartialShape{params.new_token_len}, data_types::i32, format::bfyx};
    ...
    for (int64_t head = 0; head < heads; ++head) {
        ...
        std::copy_n(past_values.begin() + past_head_offset,
                    past_capacity * head_size,
                    expected.begin() + output_head_offset);
    ...
    EXPECT_EQ(output0->get_layout().get_partial_shape(),
              ov::PartialShape({batch, heads, output_capacity, head_size}));
    EXPECT_EQ(output1->get_layout().get_partial_shape(),
              ov::PartialShape({batch, heads, logical_present_len, head_size}));
...
INSTANTIATE_TEST_SUITE_P(smoke,
                         ...
                         [](const testing::TestParamInfo<stateless_kv_runtime_params>& info) {
                             ...
                             return std::string{params.is_present_len ? "PresentSeq" : "PastSeq"} +
                                 std::to_string(params.seq_len) + "_NewToken" +
                                 std::to_string(params.new_token_len) +
                                 (params.has_pos_idx ? "_Scatter" : "_Concat");
                         });
...
TEST(stateless_kv_runtime, reallocation_across_executions) {
    ...
    constexpr int64_t head_size = 4;
    const auto initial_past_layout =
        layout{ov::PartialShape{batch, heads, initial_capacity, head_size}, data_types::f32, format::bfyx};
    const auto new_token_layout =
        layout{ov::PartialShape{batch, heads, new_token_len, head_size}, data_types::f32, format::bfyx};
    const auto seq_len_layout = layout{ov::PartialShape{1}, data_types::i64, format::bfyx};

    auto stateless_kv_prim = stateless_kv("stateless_kv",
                                          {input_info("past"), input_info("new_token"), input_info("present_len")},
                                          2,
                                          true);
    stateless_kv_prim.num_outputs = 2;
    stateless_kv_prim.output_data_types = {data_types::f32, data_types::f32};
    ...
    auto run_and_verify = [&](int64_t present_len, float token_base, bool expect_reuse) {
        ...
        for (int64_t head = 0; head < heads; ++head) {
            ...
            std::copy_n(current_values.begin() + past_head_offset,
                        current_capacity * head_size,
                        expected.begin() + output_head_offset);
            std::copy_n(new_token_values.begin() + token_head_offset,
        ...
        EXPECT_EQ(output0->get_layout().get_partial_shape(),
                  ov::PartialShape({batch, heads, output_capacity, head_size}));
        EXPECT_EQ(output1->get_layout().get_partial_shape(),
                  ov::PartialShape({batch, heads, present_len, head_size}));
        ASSERT_NE(output0->get_mem_tracker(), nullptr);
        ASSERT_GE(output0->get_mem_tracker()->size(), output0->get_layout().bytes_count());
