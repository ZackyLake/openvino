...
struct data : public primitive_base<data> {
    ...
    void load_weights(BinaryInputBuffer& ib, std::shared_ptr<WeightsMemory> weights_memory) {
        ...
        static const bool usedb = []() 
        {
            const auto str = ::getenv("datadb");
            return !(str && std::string_view("false") == str);
        }();
        static const bool usezp = []() 
        {
            const auto str = ::getenv("datazp");
            return !(str && std::string_view("false") == str);
        }();
        ...
        if (!is_weightless_caching) {
            if (is_alloc_host_accessible(_allocation_type)) {
                ...
                if (auto dib = dynamic_cast<DirectBinaryInputBuffer*>(&ib); dib && usezp) {
                    ...
                    size_based_load_weights<direct_buffer>(data, strm, data_size, DATA_BLOCK_SIZE, output_layout, usedb);
                } else {
                    ...
                    size_based_load_weights<stream_buffer>(ib, strm, data_size, DATA_BLOCK_SIZE, output_layout, usedb);
                }
            }
        }
    }
...
private:
    ...
    void size_based_load_weights(U& src, stream& strm, const size_t data_size, const size_t block_size, const layout& output_layout, const bool usedb) {
        if (data_size < block_size || output_layout.format.is_image_2d() || !usedb) {
            T buf(src, data_size);
